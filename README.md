...
* So now, the only way criminals or government can get your money is if you give it to them. You can still be [tricked into providing your seed](https://github.com/johnshearing/PrivateKeyVault#preventing-the-evil-maid-or-maid-in-the-middle-attack) or you can be tortured until you reveal it, but with the added protection of smart contracts and multi-signature wallets (to be covered in coming tutorials) neither trickery nor force will not work. Furthermore, as will be discussed in the next section, the IOTA Tangle is a completely transparent public ledger. Everyone can see where each IOTA comes from and where it goes. So there is no point in stealing it because (unlike cash) [criminals can not spend it without getting caught](https://youtu.be/507wn9VcSAE). As an added precausion one should make all spending addresses that carry a balance readily available to law enforcement. This way if the owner of the addresses should go missing then it will be that much easier for police to track the movement of your IOTAs  
